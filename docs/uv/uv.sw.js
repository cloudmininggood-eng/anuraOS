		...
		p = class extends d.EventEmitter {
			...
			async fetch({ request: e }) {
				...
				try {
					...
					let c = await this.bareClient.fetch(o, {
							headers: i.headers,
							method: i.method,
							body: i.body,
						...
						r = new m(i, c),
						a = new h(r, null, null);
					if ((this.emit("beforemod", a), a.intercepted)) return a.returnValue;
					for (let s of w) r.headers[s] && delete r.headers[s];
					if (
						(r.headers.location &&
							(r.headers.location = t.rewriteUrl(r.headers.location)),
						e.destination === "document")
					) {
						let s = r.headers["content-disposition"];
						if (!/\s*?((inline|attachment);\s*?)filename=/i.test(s)) {
							let l = /^\s*?attachment/i.test(s) ? "attachment" : "inline",
								[v] = new URL(c.finalURL).pathname.split("/").slice(-1);
							r.headers["content-disposition"] =
								`${l}; filename=${JSON.stringify(v)}`;
						}
					}
					if (
							...
							case "worker":
								{
									let s = [
										...
										.join(",");
									((r.body = `if (!self.__uv && self.importScripts) { ${t.createJsInject(t.cookie.serialize(u, t.meta, !0), e.referrer)} importScripts(${s}); }
`),
							...
							case "iframe":
							case "document":
								k(t.meta.url, r.headers["content-type"] || "") &&
									(r.body = t.rewriteHtml(await c.text(), {
										document: !0,
	...
	function k(n, e = "") {
		return (
			(d.mime.contentType(e || n.pathname) || "text/html").split(";")[0] ===
			"text/html"
		);
	}
	var h = class {
	...
	function U(n, e) {
		...
		return (
			...
			new Response(S(String(n), e), { status: 500, headers: o })
		);
	}
})();
