...
class AltTabView {
	...
	viewWindow([app, win, index]) {
		return h(
			...
			h(
				...
				h(
					"div",
					{ class: "alttab-window-icon-container" },
					h("img", {
			...
			h(
				...
				h(
					"span",
					{ class: "alttab-window-title-text" },
	...
	view() {
		return h(
			"div",
			{
				class: use(
					this.state.active,
					(active) => "alttab-container " + (active ? "" : "alttab-hidden"),
				),
			},
			$if(
				...
				h(
					...
					use(this.state.windows, (windows) =>
						windows
							.map(([a, w], i) => [a, w, i])
							.map(this.viewWindow.bind(this)),
					),
				),
				h("div", { class: "alttab-nowindows" }, h("span", null, "No windows")),
			),
		);
	}
	constructor() {
	...
	update() {
		const windows = Object.values(anura.apps).flatMap((a) =>
			a.windows.map((w) => [a, w]),
		);
		windows.sort(
			([_appA, winA], [_appB, winB]) =>
				Number(winB.element.style.zIndex) - Number(winA.element.style.zIndex),
		);
