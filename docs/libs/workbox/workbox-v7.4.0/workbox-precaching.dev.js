...
this.workbox.precaching = (function (
    ...
    */
	...
	class PrecacheStrategy extends Strategy_js.Strategy {
		 ...
		 */
		constructor(options = {}) {
			options.cacheName = cacheNames_js.cacheNames.getPrecacheName(
				options.cacheName,
			);
			super(options);
		...
		async _handleFetch(request, handler) {
			...
			if (this._fallbackToNetwork) {
				{
					logger_js.logger.warn(
						`The precached response for ` +
							`${getFriendlyURL_js.getFriendlyURL(request.url)} in ${this.cacheName} was not ` +
					...
					const wasCached = await handler.cachePut(request, response.clone());
					{
						if (wasCached) {
							logger_js.logger.log(
								`A response for ${getFriendlyURL_js.getFriendlyURL(request.url)} ` +
			...
			}
			{
				...
				logger_js.logger.log(
					`Serving the precached url: ${getFriendlyURL_js.getFriendlyURL(cacheKey instanceof Request ? cacheKey.url : cacheKey)}`,
				);
	...
	PrecacheStrategy.copyRedirectedCacheableResponsesPlugin = {
		async cacheWillUpdate({ response }) {
			return response.redirected
				? await copyResponse_js.copyResponse(response)
				: response;
		},
	};
