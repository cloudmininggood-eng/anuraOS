...
this.workbox.strategies = (function (
	...
	cacheNames_js,
	getFriendlyURL_js,
	cacheMatchIgnoreParams_js,
	Deferred_js,
	executeQuotaErrorCallbacks_js,
	timeout_js,
) {
	"use strict";
	...
	class StrategyHandler {
				...
				if (possiblePreloadResponse) {
					{
						logger_js.logger.log(
							`Using a preloaded navigation response for ` +
								`'${getFriendlyURL_js.getFriendlyURL(request.url)}'`,
						);
					}
			...
			try {
				...
				if ("dev" !== "production") {
					logger_js.logger.debug(
						`Network request for ` +
							`'${getFriendlyURL_js.getFriendlyURL(request.url)}' returned a response with ` +
			...
			} catch (error) {
				{
					logger_js.logger.log(
						`Network request for ` +
							`'${getFriendlyURL_js.getFriendlyURL(request.url)}' threw an error.`,
						error,
					);
				}
		...
		async cachePut(key, response) {
			...
			const effectiveRequest = await this.getCacheKey(request, "write");
			{
				...
				if (vary) {
					logger_js.logger.debug(
						`The response for ${getFriendlyURL_js.getFriendlyURL(effectiveRequest.url)} ` +
			...
			if (!response) {
				{
					logger_js.logger.error(
						`Cannot cache non-existent response for ` +
							`'${getFriendlyURL_js.getFriendlyURL(effectiveRequest.url)}'.`,
					);
				}
			...
			if (!responseToCache) {
				{
					logger_js.logger.debug(
						`Response '${getFriendlyURL_js.getFriendlyURL(effectiveRequest.url)}' ` +
							`will not be cached.`,
						responseToCache,
					);
				}
			...
			const oldResponse = hasCacheUpdateCallback
				? await cacheMatchIgnoreParams_js.cacheMatchIgnoreParams(
				...
				: null;
			{
				logger_js.logger.debug(
					`Updating the '${cacheName}' cache with a new Response ` +
						`for ${getFriendlyURL_js.getFriendlyURL(effectiveRequest.url)}.`,
				);
			}
			try {
				await cache.put(
    ...
    */
	...
	class Strategy {
		...
		constructor(options = {}) {
			 ...
			 * @type {string}
			 */
			this.cacheName = cacheNames_js.cacheNames.getRuntimeName(
				options.cacheName,
			);
			/**
			 * The list
		...
		async _getResponse(handler, request, event) {
			...
			try {
				...
				if (!response) {
					...
					logger_js.logger.log(
						`While responding to '${getFriendlyURL_js.getFriendlyURL(request.url)}', ` +
	...
	class NetworkOnly extends Strategy {
		...
		async _handle(request, handler) {
			...
			try {
				...
				if (this._networkTimeoutSeconds) {
					const timeoutPromise = timeout_js.timeout(
						this._networkTimeoutSeconds * 1000,
					);
