	...
	(this.workbox.strategies = (function (t, e, s, r, n, i, a, o, c) {
		...
		class l {
			constructor(t, e) {
				((this.l = {}),
					...
					(this.event = e.event),
					(this.u = t),
					(this.p = new n.Deferred()),
					(this.m = []),
					(this.v = [...t.plugins]),
					(this.q = new Map()));
				for (const t of this.v) this.q.set(t, {});
			...
			async fetch(t) {
				...
				try {
					...
					t = await fetch(
						r,
						"navigate" === r.mode ? void 0 : this.u.fetchOptions,
					);
			...
			async cacheMatch(t) {
				const e = h(t);
				let s;
				const { cacheName: r, matchOptions: n } = this.u,
			...
			async cachePut(t, s) {
				...
				const l = await this.g(s);
				if (!l) return !1;
				const { cacheName: w, matchOptions: u } = this.u,
			...
			*iterateCallbacks(t) {
			...
			waitUntil(t) {
				return (this.m.push(t), t);
			}
			async doneWaiting() {
				for (; this.m.length; ) {
					const t = this.m.splice(0),
		...
		return (
			...
			(t.NetworkFirst = class extends w {
				constructor(t = {}) {
					(super(t),
						...
						(this.R = t.networkTimeoutSeconds || 0));
				}
				async _handle(t, s) {
					const r = [],
						n = [];
					...
					const a = this.O({ timeoutId: i, request: t, logs: r, handler: s });
					n.push(a);
				...
				async O({ timeoutId: t, request: e, logs: s, handler: r }) {
					let n, i;
					try {
			...
			(t.NetworkOnly = class extends w {
				constructor(t = {}) {
					(super(t), (this.R = t.networkTimeoutSeconds || 0));
				}
				async _handle(t, s) {
					let r, n;
					try {
						const e = [s.fetch(t)];
						if (this.R) {
							const t = c.timeout(1e3 * this.R);
							e.push(t);
						}
						if (((n = await Promise.race(e)), !n))
							throw new Error(
								`Timed out the network response after ${this.R} seconds.`,
							);
					} catch (t) {
						t instanceof Error && (r = t);
					}
