((this.workbox = this.workbox || {}),
	(this.workbox.precaching = (function (t, e, s, n, i, c, r, o) {
		"use strict";
		try {
		...
		function a(t) {
			if (!t)
				throw new s.WorkboxError("add-to-cache-list-unexpected-type", {
					entry: t,
				});
			if ("string" == typeof t) {
			...
			constructor({ precacheController: t }) {
				((this.cacheKeyWillBeUsed = async ({ request: t, params: e }) => {
					const s =
						...
						this.U.getCacheKeyForURL(t.url);
					return s ? new Request(s, { headers: t.headers }) : t;
				}),
					(this.U = t));
			}
		}
		class u extends c.Strategy {
			constructor(t = {}) {
				((t.cacheName = e.cacheNames.getPrecacheName(t.cacheName)),
					super(t),
			...
			async I(t, e) {
					...
					});
				{
					...
					((n = await e.fetch(
						new Request(t, {
							integrity: "no-cors" !== t.mode ? c || s : void 0,
						}),
					)),
						s &&
							r &&
			...
			P() {
				...
				for (const [s, n] of this.plugins.entries())
					n !== u.copyRedirectedCacheableResponsesPlugin &&
						(n === u.defaultPrecacheCacheabilityPlugin && (t = s),
						n.cacheWillUpdate && e++);
				0 === e
		...
		((u.defaultPrecacheCacheabilityPlugin = {
			...
			(u.copyRedirectedCacheableResponsesPlugin = {
				cacheWillUpdate: async ({ response: t }) =>
					t.redirected ? await i.copyResponse(t) : t,
			}));
		class f {
			constructor({
				cacheName: t,
				...
				((this.F = new Map()),
					...
					(this.S = new Map()),
					(this.u = new u({
						cacheName: e.cacheNames.getPrecacheName(t),
						plugins: [...s, new l({ precacheController: this })],
						fallbackToNetwork: n,
					})),
			...
			addToCacheList(t) {
				...
				for (const n of t) {
					...
					const { cacheKey: t, url: i } = a(n),
						c = "string" != typeof n && n.revision ? "reload" : "default";
					if (this.F.has(i) && this.F.get(i) !== t)
						throw new s.WorkboxError("add-to-cache-list-conflicting-entries", {
							firstEntry: this.F.get(i),
							secondEntry: t,
						});
					if ("string" != typeof n && n.integrity) {
						if (this.S.has(t) && this.S.get(t) !== n.integrity)
							throw new s.WorkboxError(
								"add-to-cache-list-conflicting-integrities",
								{ url: i },
							);
						this.S.set(t, n.integrity);
					}
					if ((this.F.set(i, t), this.B.set(i, c), e.length > 0)) {
			...
			createHandlerBoundToURL(t) {
				...
				if (!e) throw new s.WorkboxError("non-precached-url", { url: t });
				return (s) => (
		...
		return (
			(t.PrecacheController = f),
			(t.PrecacheFallbackPlugin = class {
				constructor({ fallbackURL: t, precacheController: e }) {
					((this.handlerDidError = () => this.U.matchPrecache(this.D)),
						(this.D = t),
						(this.U = e || d()));
				}
			}),
			(t.PrecacheRoute = y),
			(t.PrecacheStrategy = u),
			(t.addPlugins = function (t) {
				d().strategy.plugins.push(...t);
			}),
			(t.addRoute = p),
			(t.cleanupOutdatedCaches = function () {
				self.addEventListener("activate", (t) => {
					const s = e.cacheNames.getPrecacheName();
					t.waitUntil(
						(async (t, e = "-precache-") => {
							...
							return (
								await Promise.all(s.map((t) => self.caches.delete(t))),
								s
							);
						})(s).then((t) => {}),
					);
			...
			(t.createHandlerBoundToURL = function (t) {
				return d().createHandlerBoundToURL(t);
			}),
			(t.getCacheKeyForURL = function (t) {
				return d().getCacheKeyForURL(t);
			}),
			(t.matchPrecache = function (t) {
				return d().matchPrecache(t);
			}),
			(t.precache = R),
