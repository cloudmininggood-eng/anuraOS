...
this.workbox.recipes = (function (
	exports,
	registerRoute_js,
	StaleWhileRevalidate_js,
	CacheFirst_js,
	CacheableResponsePlugin_js,
	ExpirationPlugin_js,
	NetworkFirst_js,
	setCatchHandler_js,
	matchPrecache_js,
    ...
    */
	...
	function googleFontsCache(options = {}) {
		...
		registerRoute_js.registerRoute(
			...
			new StaleWhileRevalidate_js.StaleWhileRevalidate({
		...
		registerRoute_js.registerRoute(
			...
			new CacheFirst_js.CacheFirst({
				cacheName: fontCacheName,
				plugins: [
					new CacheableResponsePlugin_js.CacheableResponsePlugin({
						statuses: [0, 200],
					}),
    ...
    */
	...
	function offlineFallback(options = {}) {
		...
		const handler = async (options) => {
			...
			if (dest === "document") {
				const match =
					(await matchPrecache_js.matchPrecache(pageFallback)) ||
					(await cache.match(pageFallback));
				return match || Response.error();
			}
			if (dest === "image" && imageFallback !== false) {
				const match =
					(await matchPrecache_js.matchPrecache(imageFallback)) ||
					(await cache.match(imageFallback));
				return match || Response.error();
			}
			if (dest === "font" && fontFallback !== false) {
				const match =
					(await matchPrecache_js.matchPrecache(fontFallback)) ||
