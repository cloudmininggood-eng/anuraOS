	...
	(this.workbox.routing = (function (t, e) {
		...
		class i {
			...
			handleRequest({ request: t, event: e }) {
				...
				let o = i && i.handler;
				const u = t.method;
				if ((!o && this.L.has(u) && (o = this.L.get(u)), !o)) return;
				let c;
				try {
					c = o.handle({ url: s, request: t, event: e, params: n });
				} catch (t) {
					c = Promise.reject(t);
				}
				...
				return (
					c instanceof Promise &&
						...
						(c = c.catch(async (r) => {
							...
							if (this.H)
								return this.H.handle({ url: s, request: t, event: e });
							throw r;
						})),
					c
				);
			}
			findMatchingRoute({ url: t, sameOrigin: e, request: s, event: r }) {
				const n = this.C.get(s.method) || [];
				for (const i of n) {
					let n;
					const o = i.match({ url: t, sameOrigin: e, request: s, event: r });
					if (o)
						return (
							(n = o),
			...
			registerRoute(t) {
				(this.C.has(t.method) || this.C.set(t.method, []),
					this.C.get(t.method).push(t));
			}
			unregisterRoute(t) {
				if (!this.C.has(t.method))
					throw new e.WorkboxError(
						"unregister-route-but-not-found-with-method",
						{ method: t.method },
					);
				const s = this.C.get(t.method).indexOf(t);
				if (!(s > -1))
					throw new e.WorkboxError("unregister-route-route-not-registered");
				this.C.get(t.method).splice(s, 1);
			}
		}
		let o;
		const u = () => (
		...
		return (
			(t.NavigationRoute = class extends r {
				constructor(t, { allowlist: e = [/./], denylist: s = [] } = {}) {
					(super((t) => this.j(t), t), (this.M = e), (this.N = s));
				}
				j({ url: t, request: e }) {
					if (e && "navigate" !== e.mode) return !1;
					const s = t.pathname + t.search;
					for (const t of this.N) if (t.test(s)) return !1;
			...
			(t.Route = r),
			(t.Router = i),
			(t.registerRoute = function (t, s, i) {
				let o;
				if ("string" == typeof t) {
					const e = new URL(t, location.href);
					o = new r(({ url: t }) => t.href === e.href, s, i);
