...
function o(e, t = globalThis, n = ["*"]) {
	(t.addEventListener("message", function s(i) {
		...
		const { id: u, type: l, path: p } = Object.assign({ path: [] }, i.data),
			f = (i.data.argumentList || []).map(w);
		let g;
		try {
			...
			switch (l) {
				case "GET":
					...
					break;
				case "SET":
					((t[p.slice(-1)[0]] = w(i.data.value)), (g = !0));
					break;
				case "APPLY":
					g = n.apply(t, f);
					break;
		...
		Promise.resolve(g)
			...
			.then((n) => {
				const [a, i] = v(n);
				(t.postMessage(Object.assign(Object.assign({}, a), { id: u }), i),
					"RELEASE" === l &&
...
function p(e) {
	return S(e, new Map(), { type: "RELEASE" }).then(() => {
		c(e);
	});
}
const f = new WeakMap(),
	...
	const o = new Proxy(s, {
		...
		set(t, n, s) {
			...
			return S(
				e,
				r,
				{ type: "SET", path: [...a, n].map((e) => e.toString()), value: o },
				c,
			).then(w);
		},
		apply(n, s, o) {
			...
			if (c === t) return S(e, r, { type: "ENDPOINT" }).then(w);
...
function S(e, t, n, r) {
	return new Promise((a) => {
		...
		(t.set(s, a),
			e.start && e.start(),
			e.postMessage(Object.assign({ id: s }, n), r));
	});
}
export {
	t as createEndpoint,
