...
export const libcurl = (function () {
	...
	var _http_set_options = (Module["_http_set_options"] =
		createExportWrapper("http_set_options"));
	var _free = (Module["_free"] = createExportWrapper("free"));
	var _http_set_cookie_jar = (Module["_http_set_cookie_jar"] =
		createExportWrapper("http_set_cookie_jar"));
	var _http_get_info = (Module["_http_get_info"] =
		createExportWrapper("http_get_info"));
	var _create_request = (Module["_create_request"] =
		createExportWrapper("create_request"));
	var _request_cleanup = (Module["_request_cleanup"] =
		createExportWrapper("request_cleanup"));
	var _request_set_proxy = (Module["_request_set_proxy"] =
		createExportWrapper("request_set_proxy"));
	var _session_create = (Module["_session_create"] =
		createExportWrapper("session_create"));
	var _session_perform = (Module["_session_perform"] =
		createExportWrapper("session_perform"));
	var _session_set_options = (Module["_session_set_options"] =
		createExportWrapper("session_set_options"));
	var _session_add_request = (Module["_session_add_request"] =
		createExportWrapper("session_add_request"));
	var _session_get_active = (Module["_session_get_active"] =
		createExportWrapper("session_get_active"));
	var _session_remove_request = (Module["_session_remove_request"] =
		createExportWrapper("session_remove_request"));
	var _session_cleanup = (Module["_session_cleanup"] =
		createExportWrapper("session_cleanup"));
	var _recv_from_socket = (Module["_recv_from_socket"] =
		createExportWrapper("recv_from_socket"));
	var _send_to_socket = (Module["_send_to_socket"] =
		createExportWrapper("send_to_socket"));
	var _tls_socket_set_options = (Module["_tls_socket_set_options"] =
		createExportWrapper("tls_socket_set_options"));
	var _get_version = (Module["_get_version"] =
		createExportWrapper("get_version"));
	var _get_error_str = (Module["_get_error_str"] =
		createExportWrapper("get_error_str"));
	var _recv_from_websocket = (Module["_recv_from_websocket"] =
		createExportWrapper("recv_from_websocket"));
	var _send_to_websocket = (Module["_send_to_websocket"] =
		createExportWrapper("send_to_websocket"));
	var _close_websocket = (Module["_close_websocket"] =
		createExportWrapper("close_websocket"));
	var _websocket_set_options = (Module["_websocket_set_options"] =
		createExportWrapper("websocket_set_options"));
	var _get_result_size = (Module["_get_result_size"] =
		createExportWrapper("get_result_size"));
	var _get_result_buffer = (Module["_get_result_buffer"] =
		createExportWrapper("get_result_buffer"));
	var _get_result_code = (Module["_get_result_code"] =
		createExportWrapper("get_result_code"));
	var _get_result_closed = (Module["_get_result_closed"] =
		createExportWrapper("get_result_closed"));
	var _get_result_bytes_left = (Module["_get_result_bytes_left"] =
		createExportWrapper("get_result_bytes_left"));
	var _get_result_is_text = (Module["_get_result_is_text"] =
		createExportWrapper("get_result_is_text"));
	...
	var dynCall_iiiiijjii = (Module["dynCall_iiiiijjii"] =
		createExportWrapper("dynCall_iiiiijjii"));
	...
	if (wasmBinaryFile && isDataURI(wasmBinaryFile)) run(); //extra client code goes here
	/* __extra_libraries__ */
...
`;
	class CurlSession {
		constructor(options = {}) {
			...
			this.headers_callback_ptr = Module.addFunction(
				(request_id, chunk_ptr, chunk_size) => {
					this.headers_callback(request_id, chunk_ptr, chunk_size);
				},
				"viii",
			);
			this.data_callback_ptr = Module.addFunction(
				(request_id, chunk_ptr, chunk_size) => {
	...
	class HTTPSession extends CurlSession {
		...
		request_async(url, params, body) {
			return new Promise((resolve, reject) => {
				...
				let headers_callback = (stream) => {
					...
					let response = this.constructor.create_response(
						stream,
				...
				let finish_callback = (error) => {
					...
					if (error > 0) {
						error_msg(
							`Request "${url}" failed with error code ${error}: ${get_error_str(error)}`,
						);
						reject(
							new TypeError(
								`Request failed with error code ${error}: ${get_error_str(error)}`,
							),
						);
					} else if (error === -1) {
				...
				http_handle = this.stream_response(
					url,
					headers_callback,
					finish_callback,
					params.signal,
				);
				c_func(_http_set_options, [
					http_handle,
					params_json,
					body_ptr,
					body_length,
		...
		async fetch(resource, params_old = {}) {
			...
			if (resource instanceof Request) {
				...
				if (resource_body.byteLength !== 0) params.body = resource_body;
		...
		static create_response(response_data, response_info) {
			response_info.ok =
				response_info.status >= 200 && response_info.status < 300;
	...
	class CurlWebSocket extends CurlSession {
		...
		connect() {
			...
			if (this.protocols) {
				...
				request_options.headers["Sec-Websocket-Protocol"] =
					this.protocols.join(", ");
			}
			...
			this.http_handle = this.create_request(
				this.url,
				data_callback,
				finish_callback,
				headers_callback,
		...
		cleanup(error = 0) {
			...
			if (this.recv_loop) clearInterval(this.recv_loop);
			...
			if (error) {
				error_msg(
					`Websocket "${this.url}" encountered error code ${error}: ${get_error_str(error)}`,
				);
				this.onerror(error);
			} else {
				this.onclose();
		...
		connect() {
			...
			this.socket.onmessage = (data) => {
				...
				if (typeof data === "string") {
					...
					//binary frame received as uint8array
					if (this.binaryType == "blob") {
	...
	function set_websocket_url(url) {
		websocket_url = url;
		if (typeof Module.websocket === "undefined") Module.websocket = {};
