			...
			h("input", {
				...
				"on:dragend": (e) => {
					if (!this.insidedrag) {
						...
						anura.settings.set(
							"applist",
							anura.settings.get("applist").filter((p) => p !== app.package),
						);
						this.updateTaskbar();
					} else {
				...
				"on:drag": (e) => {
					...
					icons[closestIndex].style.borderRight = "2px solid var(--theme-fg)";
				},
				class: "showDialog",
				"on:click": (e) => {
		...
		this.element = h(
			...
			h(
				...
				h(
					"ul",
					null,
					use(this.state.pinnedApps, (apps) =>
				...
				h(
					"ul",
					null,
					use(this.state.activeApps, (apps) =>
						apps.map(this.shortcut.bind(this)),
					),
				),
			),
