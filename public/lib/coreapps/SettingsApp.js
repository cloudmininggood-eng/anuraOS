...
class SettingsApp extends App {
	...
	page = async () =>
		h(
			...
			h("div", { class: "header" }, h("h2", null, "Anura Settings")),
			h(
				"div",
				{ css: this.state, class: "container" },
				h(
					"div",
					{ class: "sidebar" },
					h(
						...
						h(
							"span",
							{ class: "sidebar-settings-item-name" },
				...
				h(
					"div",
					{ "bind:this": use(this.state.settingsBody), class: "settings-body" },
					h(
						"div",
						{ id: "general", class: "general settings-category" },
								...
								callback: async () => {
									if (
										...
										await window.idbKeyval.set(
											"bootFromOPFS",
											!(await window.idbKeyval.get("bootFromOPFS")),
										);
										window.location.reload();
									} else {
										anura.settings.set(
					...
					h(
						"div",
						{ id: "v86", class: "v86 settings-category" },
										...
										h(
											...
											h(
												...
												h("input", {
													...
													id: "disk-size-bytes",
													value: Math.ceil(anura.x86hdd.size / 1000000) || 0,
													type: "text",
												}),
											),
											h(
													...
													"on:click": async () => {
														...
														if (document.getElementById("disk-size-bytes")) {
															await anura.x86hdd.resize(
																parseInt(
																	document.getElementById("disk-size-bytes")
																		.value,
																) * 1000000,
															);
										...
										h(
											...
											h(
												"button",
												{
													...
													"on:click": async () => {
														const name =
															document.getElementById("xappstub-name").value;
														const cmd =
															document.getElementById("xappstub-cmd").value;
														if (name && cmd) {
					...
					h(
						"div",
						{ id: "reset", class: "settings-category" },
