...
class TaskManager extends App {
						...
						use(anura.processes.state.procs, (procs) =>
							procs.map((proc) =>
								h(
									...
									h("td", null, h("span", null, "Measuring")),
									h(
										"td",
										{ class: "pid" },
					...
					"button",
					{
						...
						"on:click": () => {
							const proc = anura.processes.state.procs.find(
								(proc) => proc && proc.deref()?.pid === this.state.selected,
							);
							if (proc) {
								proc.deref()?.kill();
							}
	...
	async open(args = []) {
		...
		if (performance.measureUserAgentSpecificMemory) {
			const measure = async () => {
				if (!performance.measureUserAgentSpecificMemory) return {}; // this API only works on chromium
			...
			while (page.parentElement) {
				...
				const allRows = Array.from(
					document.querySelectorAll("[id^=taskmgr-proc-]").values(),
				);
				for (const app in perfList) {
					const row = page.querySelector("#taskmgr-proc-" + app);
					row.children[1].innerText = Math.ceil(perfList[app] / 1000) + "kB";
