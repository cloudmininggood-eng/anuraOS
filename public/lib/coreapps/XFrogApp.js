	...
	async spawn_xwindow(xwid) {
		...
		const sfocus = async () => {
			...
			timeout = setTimeout(() => {
				...
				if (isNew)
					anura.x86.runcmd(
						`DISPLAY=:0 xdotool search --maxdepth 1 --onlyvisible ".*" 2>/dev/null | while read wid; do DISPLAY=:0 xdotool windowunmap $wid; done; DISPLAY=:0 xdotool windowmap ${xwid}; DISPLAY=:0 xdotool windowmove ${xwid} 0 0; DISPLAY=:0 xdotool windowsize ${xwid} ${win.width} ${win.height - 28}`,
					);
				setTimeout(() => {
